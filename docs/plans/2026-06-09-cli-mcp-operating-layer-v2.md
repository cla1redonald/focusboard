...
**4b (shipped 2026-06-10, PR #34):** the web's own writes go per-card — the cards table is the system of record. Load = `app_state` blob (non-card state) + cards rows; save = per-card diff against a last-seen-server snapshot (retry-idempotent upsert inserts, version-CAS updates, version-guarded deletes; CAS miss → **accept-theirs** refetch + reconcile; saves serialized one-at-a-time). The blob is saved WITHOUT a cards key. Realtime split: per-card INSERT/UPDATE/DELETE from `cards` (own-write echoes dropped by version) + `app_state` UPDATE for non-card state (echoes dropped by JSON compare) — replaces the 3s wall-clock echo suppression. `fb_add_card`/`fb_mutate_card` write rows directly (CAS bumps the version in-function; 409 contract unchanged); the blob→rows trigger became conditional (no-ops when the blob lacks a cards key) so the migration applies safely BEFORE the deploy; the cleanup migration (`20260610130000_retire_blob_cards.sql`, after runtime verification) drops it and strips legacy blob cards arrays. The residual full-blob clobber race is CLOSED. Demo mode (no Supabase) unchanged.

**4b runtime-verification findings (2026-06-10):** the real-browser pass (dedicated smoke account, CLI mutating alongside two open tabs) caught a LATENT PROD BUG: `app_state` was never in the `supabase_realtime` publication, so its postgres_changes binding had been silently rejected since Phase 0 — the pre-4b board realtime sync NEVER worked in prod (only capture-inbox realtime did), and the failing binding killed 4b's combined channel, silencing card events too. Fixed by (1) adding `app_state` to the publication (migration `20260610120000`), (2) splitting cards/app_state onto SEPARATE channels, (3) logging subscription status errors instead of bare `.subscribe()`. After the fix: CLI add/move/done appear live in open tabs, two-tab web-write convergence confirmed, 409 STALE_STATE verified on prod, cleanup migration `20260610130000` applied (trigger dropped, blob cards arrays stripped). Verification now runs against the `focusboard-smoke` account (`scripts/smoke-account.sh`) — never Claire's account.
