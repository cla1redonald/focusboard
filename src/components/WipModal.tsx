...
export function WipModal({
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center">
      <div className="absolute inset-0 bg-emerald-950/30 backdrop-blur-sm" onClick={onCancel} />
      <div className="relative w-[520px] max-w-[92vw] rounded-2xl border border-emerald-700/15 bg-white/95 p-6 shadow-[0_30px_80px_rgba(0,0,0,0.2)]">
        <div className="display-font text-xl text-emerald-950">{title}</div>
        <div className="mt-2 text-sm text-emerald-900/70">{message}</div>

        {askReason && (
          <div className="mt-4">
            <label className="text-xs text-emerald-900/60">
              {reasonLabel ?? "Reason"}
            </label>
            <input
              autoFocus
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button
            ...
            className="rounded-full bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-[0_10px_30px_rgba(16,185,129,0.25)] transition hover:-translate-y-0.5 hover:bg-emerald-700 disabled:opacity-50"
          >
            {confirmText}
          </button>
        </div>
